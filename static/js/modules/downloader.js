...
import { getAlbumTracksAPI, getTrackInfo, getItemInfoAPI, getLyricsAPI } from './api.js';
import { settingsManager } from './settings.js';
import { loadFFmpeg, applyMetadata, mergeHiresSegments } from './ffmpeg.js';
    ...
    async processNext() {
        ...
        try {
            ...
            if (err.name === 'AbortError') {
                ...
                updateDownloadManagerStatus({ trackStatus: `任务 "${this.currentTask.name}" 失败: ${err.message}`, autoHide: true });
            }
        }
...
export async function downloadAlbum(albumItem) {
    ...
    try {
        ...
        updateDownloadManagerStatus({ 
            albumTitle: `获取失败: ${albumItem.title}`, 
            trackStatus: err.message, 
            autoHide: true 
        });
        return; 
    }
